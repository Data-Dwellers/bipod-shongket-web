...
import { getSOSRequests, createSOSRequest, updateSOSRequest } from "@/services/sosRequestService";
...
export default function Page() {
        ...
        const fetchSOSRequests = async () => {
            try {
                ...
                if (result && result.data) {
                    ...
                    if (user && user.location && user.location.lat && user.location.long) {
    ...
    const handleRespond = async (sosRequest) => {
        ...
        try {
            const name = await getLocationName(sosRequest.location.lat, sosRequest.location.long);
        ...
        try {
            if (activeSOS) {
                ...
                const currentLocation = userLocation ||
                    (user && user.location ? { lat: user.location.lat, long: user.location.long } : null);

                if (!currentLocation) {
                    alert("Cannot send SOS: Location not available. Please enable location services.");
        ...
        return (
            <div className="flex justify-center items-center p-10">
                <Card className="w-full max-w-xl">
                    <CardHeader>
                        <CardTitle className="text-center text-red-600">Location Not Available</CardTitle>
                    </CardHeader>
                    <div className="p-6 text-center">
                        <p>Please update your profile with your location to see nearby SOS requests.</p>
                    </div>
                </Card>
            </div>
        );
    ...
    return (
        <div className="p-6">
            ...
            <h1 className="text-3xl font-bold mb-6 text-red-600">SOS Requests Within Your Range</h1>
            ...
            {nearbyRequests.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {nearbyRequests.map((request) => (
                        <Card key={request._id} className="border-red-500 shadow-md hover:shadow-lg transition-shadow">
                            ...
                            <CardContent className="pt-6">
                                <div className="space-y-2">
                                    <p className="text-sm text-gray-500 dark:text-gray-400">
                                        Sent by: <span className="font-semibold">{request.user}</span>
                                    </p>
                                    <p className="text-sm text-gray-500 dark:text-gray-400">
                                        Date: <span className="font-semibold">{new Date(request.date).toLocaleString()}</span>
                                    </p>
                                </div>
                            </CardContent>
                            <CardFooter>
            ...
            <AlertDialog open={showModal} onOpenChange={setShowModal}>
                <AlertDialogContent className="max-w-md">
                    <AlertDialogHeader>
                        <AlertDialogTitle className="text-red-600">Emergency SOS Details</AlertDialogTitle>
                        <AlertDialogDescription>
                            {selectedSOS && (
                                <div className="space-y-4 mt-4">
                                    <div className="p-4 bg-red-50 dark:bg-red-900/20 rounded-lg">
                                        <p className="text-sm font-medium">
                                            Sent by: <span className="font-bold">{selectedSOS.user}</span>
                                        </p>
                                        <p className="text-sm font-medium">
                                            Date: <span className="font-bold">{new Date(selectedSOS.date).toLocaleString()}</span>
                                        </p>
                                    </div>
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <p className="text-sm">
                                            <span className="font-medium">Latitude:</span> {selectedSOS.location.lat}
                                        </p>
                                        <p className="text-sm">
                                            <span className="font-medium">Longitude:</span> {selectedSOS.location.long}
                                        </p>
                                        <div className="pt-2">
                                            <p className="text-sm font-medium">Location Name:</p>
                                            {locationLoading ? (
                                                <p className="text-sm italic">Loading location name...</p>
                                            ) : (
